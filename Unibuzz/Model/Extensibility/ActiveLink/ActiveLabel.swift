...
@IBDesignable open class ActiveLabel: UILabel {
    ...
    @IBInspectable open var mentionColor: UIColor = Colors.blue {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var mentionSelectedColor: UIColor? = Colors.blue {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var hashtagColor: UIColor = Colors.blue {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var hashtagSelectedColor: UIColor? = Colors.blue {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var URLColor: UIColor = Colors.blue {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var URLSelectedColor: UIColor? = Colors.blue {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    fileprivate func addLinkAttribute(_ mutAttrString: NSMutableAttributedString) {
        ...
        for (type, elements) in activeElements {
            ...
            case .custom: attributes[NSAttributedString.Key.foregroundColor] = customColor[type] ?? defaultCustomColor
						...
						//attributes[NSAttributedString.Key.underlineStyle] = hightlightFont
						/*
						at.addAttribute(NSAttributedString.Key.underlineStyle, value: 0, range: range)
								attributes[NSAttributedString.Key.underlineStyle] = []
								attributes[NSAttributedString.Key.underlineStyle] = NSUnderlineStyle.none styleNone.rawValue
						*/
            ...
            for element in elements {
				...
				mutAttrString.addAttribute(NSAttributedString.Key.underlineStyle, value: 0, range: range)
        }
    }
